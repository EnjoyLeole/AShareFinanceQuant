...
class Config(object):
    ...
    def get_project_candidates(cls):
        select = "select pid from %s where %s is NULL or %s <%s" % (
            cls.db_handler.prefix_name(cls.project_table), cls.db_handler.prefix_name("ifEmailed"),
            cls.db_handler.prefix_name("ifEmailed"), cls.upper_flag)
    ...
    def _get_setting(glass, if_wide = False):
        ...
        for field in glass.__dict__:
            ...
            if if_wide:
                ...
                value_type = (int, float, str, ClassProperty, np.int64)
