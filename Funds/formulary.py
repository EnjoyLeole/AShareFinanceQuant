...
class _FinancialFormula(metaclass = SingletonMeta):
    ...
    def __init__(self):
        ...
        self.formula_by_type['stock'] = self.table[
            self.table.target.apply(lambda val: INDEX_FLAG not in val)]
        self.formula_by_type['index'] = self.table[
            self.table.target.apply(lambda val: INDEX_FLAG in val)]

        self.target_savings = \
            [tar for tar in self.formula_by_type['stock'].target if tar not in ['E', 'FI']] \
            + [p + PERCENTILE for p in self.key_targets] \
            + [s + STD_DISTANCE for s in self.key_targets]
    ...
    def calculate(self, data, formula, prelude, target_calculator):
        ...
        if target in data:
            ...
            def _prelude(data, formula, prelude):
                ...
                fields = self.equation_parameters[formula.target]
                ...
                if prelude != prelude:
                    ...
                    def target_calc(df, field, new_field, n):
                        target_calculator(field)
...
class Ticks(object):
    ...
    def __init__(self, code, table_getter: callable, security_type: str,
                 refer_index = None):
        ...
        def saved_target_retrieve():
            targets = DMgr.read(self.target_category, code)
    ...
    def calc_target(self, formula = None, target: str = None, prelude = np.nan):
        ...
        target_calculator = lambda target: self.calc_target(target = target)
        if formula.source == TICK:
